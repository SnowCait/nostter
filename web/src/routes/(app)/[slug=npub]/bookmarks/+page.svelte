		...
		if (ids.length > 0) {
			...
			rxNostr
				...
				.subscribe((packet) => {
					...
					publicBookmarkEventItems = publicBookmarkEventItems.toSorted((a, b) => b.event.created_at - a.event.created_at);
				});
		}
					...
					if (ids.length > 0) {
						...
						rxNostr
							...
							.subscribe((packet) => {
								...
								privateBookmarkEventItems = privateBookmarkEventItems.toSorted((a, b) => b.event.created_at - a.event.created_at);
							});
					}
				})
				.catch((reason) => {
